...
const PpkMapUchastok = ({ cadastrNumber, setCloseChecker, setAlarmMessage, setCadastrNumber, flatRights, setPromoCode, promoCode, setActivate, activate, lat, lon, rightsCheck, owner, sendActivePromoCode, closeChecker, setLoading, loading, setBaloonData, baloonData, genetiveRegionName, regionName, districtsList, city, settlement, settlementName, onCkickCadastrNumber, setOnCkickCadastrNumber, setIsVisible, isVisible, error, setError, type, setType, setIsCurrentlyDrawing, isCurrentlyDrawing, setPolygonCoordinates, polygonCoordinates, setIsEditingPolygon, isEditingPolygon, setShema, shema }) => {
      ...
      <div className={style["object__block-wrap"]}>
          ...
          <div className={`${style.reestrMapContent}`} ref={ref}>
                ...
                >
                  <div className={style.info__content}>
                      {/* <div className={style["test__rightblock_btnb-img"]}> Заказать отчеты {String.fromCharCode(9660)}</div> */}
                    <div className="stack" style={{'--stacks': 3}}>
                      <span style={{'--index': 2}}>Заказать схему участка</span>
                      <span style={{'--index': 1}}>Заказать схему участка</span>
                      <span style={{'--index': 0}}>Заказать схему участка</span>
                    </div>
                  </div>
                </div>
        ...
        {(baloonData === 'error' || shema) &&
          <div className={`${style.reestrMapContent3}`}>
                ...
                <div className={style.info__content}>
                  <div className={style.info__main}>
                    <div className={style["info__main-address"]}>Если межевание не делалось, вы можете дистанционно заказать схему образования участка у специалистов кадастрового сервиса.</div>
                  </div>
                  <div style={{
                    ...
                    <ul className={style.contentText} style={{
                      ...
                      <li>Помощь в подборе участка</li>
                      <li>Сопровождение специалистом на всех этапах</li>
